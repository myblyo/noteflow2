...
import type { FilterCategory, Note, ChecklistNote, IdeaNote } from "../types";
...
const SEED_NOTES: Note[] = [
  {
    ...
    content: "Contenido de ejemplo para esta nota.",
    isFavorite: true,
    createdAt: new Date(),
    updateAt: new Date(),
  },
...
export default function FullLayout() {
  ...
  const renderNoteCard = (note: Note, isFullWidth = false) => {
    ...
    return (
      <Pressable
        key={note.id}
        onPress={() => store.setSelectedNoteId(note.id)}
        style={[
          styles.listItem,
      ...
      >
        <View style={styles.listItemContent}>
          ...
          <Text style={[styles.bodyText, { color: colors.textSecondary }]} numberOfLines={1}>
            {note.description || note.content || "Description"}
          </Text>
        </View>
  ...
  const renderIdeaRow = (idea: IdeaNote) => (
    <View key={idea.id} style={[styles.listItem, { backgroundColor: colors.surfaceTranslucent }]}>
          ...
          <>
            ...
            <View style={styles.rightColumn}>
              <View style={[styles.listContainer, styles.rightPanel, { backgroundColor: colors.surfaceSecondary }]}>
                ...
                <ScrollView showsVerticalScrollIndicator={false}>
                  {store.checklists.flatMap((c) => c.items.map((i) => renderTodoRow(c.id, i)))}
                </ScrollView>
              </View>
